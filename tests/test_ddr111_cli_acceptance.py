...
def test_cli_acceptance_create_source_host_episode_generate_export(
    ...
    corpus.write_text(
        "Deterministic local acceptance corpus about orbital mechanics.",
        encoding="utf-8",
    )
    base = ["--data-dir", str(data_dir), "--json"]

    project = _json_call(
        [*base, "project", "create", "DDR-111 acceptance"],
        capsys,
    )
    project_id = str(project["id"])
    added = _json_call(
        [*base, "source", "add", project_id, str(corpus)],
        capsys,
    )
    ...
    plan = _json_call(
        [*base, "episode", "plan", project_id, episode_id],
        capsys,
    )
    ...
    status = _json_call(
        [*base, "episode", "status", project_id, episode_id],
        capsys,
    )
...
def test_cli_acceptance_pause_resume_uses_durable_control_path(
    ...
    paused = _json_call(
        [*base, "pause", project.id, episode.id],
        capsys,
    )
    assert paused["id"] == run.id
    assert paused["state"] == "paused"
    resumed = _json_call(
        [*base, "resume", project.id, episode.id],
        capsys,
    )
