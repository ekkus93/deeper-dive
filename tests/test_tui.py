...
def test_preflight_generate_starts_pipeline_and_reuses_run(tmp_path: Path) -> None:
    asyncio.run(_preflight_generate_starts_pipeline(tmp_path))


async def _preflight_generate_starts_pipeline(tmp_path: Path) -> None:
    service = _service(tmp_path)
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        ...
        completed = service.runs(project.id).list_completed_stages(run.id)
        assert tuple(completed) == DEFAULT_STAGES
        first_run_id = run.id

        app.action_navigate("generate")
        await pilot.pause()
        _preflight(app).action_generate()
        await pilot.pause()
...
def _text(
    screen: HomeProjectsScreen | SourcesScreen | PreflightScreen | GenerationMonitorScreen,
    selector: str,
) -> str:
