...
def test_claim_can_hold_supporting_and_contradicting_evidence_without_changing_provenance(
    tmp_path,
) -> None:
    ...
    corpus.create_source(
        SourceRecord("s", "p", "user", "text", "Source", "now", locator="paper.pdf")
    )
    ...
    repository.add_evidence(
        Evidence("e1", "claim", "c1", EvidenceRelation.SUPPORTS, {"rank": 1}, {"checked": True})
    )
    repository.add_evidence(
        Evidence("e2", "claim", "c2", EvidenceRelation.CONTRADICTS, {"rank": 2}, {"checked": False})
    )
